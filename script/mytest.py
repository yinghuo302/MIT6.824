#!/usr/bin/env python
# 在https://gist.github.com/JJGO/0d73540ef7cc2f066cb535156b7cbdab修改，提前到需要测试的lab进行编译，减少耗时。但只能通过golang的-test.run指定运行哪些测试，无法以数组的形式给出
...
def run_test(test: str, race: bool, timing: bool):
    global tester_file
    cmd=f"{tester_file} -test.run={test}"
    if timing:
        cmd = "time " + cmd
...
def build_test(lab = "raft",race=False):
    # script_path = os.path.realpath()
    script_dir = os.path.dirname(__file__)
    test_dir = os.path.join(script_dir, "..", "src", lab)
    global tester_file
    tester_file = os.path.join(script_dir, "tester")
    if os.path.exists(tester_file):
        os.remove(tester_file)
    cmd = f"cd {test_dir} && go test -c -o {tester_file}"
    if race:
        cmd = f"cd {test_dir} && go test -c -race -o {tester_file}"
    result = subprocess.run(cmd,shell=True)
    if not os.path.exists(tester_file):
        print("\033[1;31mERROR: Build failed\033[0m")
        sys.exit(1)
    return 
...
# fmt: off
def run_tests(
    tests: str             = typer.Option(".",    '--tests',           '-T',    help='run only tests and examples matching regexp default all.'),
    lab: str               = typer.Option("raft", '--lab',             '-L',    help='the lab to test raft/kvraft/shardctrler/shardkv default raft'),
    sequential: bool       = typer.Option(False,  '--sequential',      '-s',    help='Run all test of each group in order'),
    workers: int           = typer.Option(4,      '--workers',         '-p',    help='Number of parallel tasks'),

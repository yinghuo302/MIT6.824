...
# 测试哪一个Lab raft,kvraft,shardctrler,shardkv
lab="raft"
if [ $# -gt 2 ]; then
	lab="$3"
fi
...
rm "${script_dir}/tester"
cd ${test_dir} && go test -c -o "${script_dir}/tester"

if ! [ -f "${script_dir}/tester" ]; then

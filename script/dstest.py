#!/usr/bin/env python
# 在https://gist.github.com/JJGO/0d73540ef7cc2f066cb535156b7cbdab修改，不需要在cd到对应路径，直接通过lab指定
import itertools
import math
import signal
import subprocess
...
def run_test(test: str, race: bool, timing: bool,test_dir):
    cmd = f"cd {test_dir} && {'time' if timing else ''} go test {'-race' if race else ''} -run={test}"
...
def run_tests(
    tests: List[str],
    lab: str               = typer.Option("raft", '--lab',             '-b',    help='Run all test of each group in order'),
                ...
                while completed < total:
                    ...
                    if n < workers:
                        for test in itertools.islice(test_instances, workers-n):
                            futures.append(executor.submit(run_test, test, race, timing,test_dir))
